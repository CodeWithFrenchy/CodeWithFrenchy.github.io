...
_Exemple de représentation Clean Architecture en cercles concentriques (modèle Onion)_. Les dépendances pointent vers le noyau du système : la partie cœur métier au centre n’a aucune dépendance sur les couches extérieures, tandis que les couches externes (UI, Infrastructure) peuvent dépendre de ce qui est plus central.
